...
// Card Component - Điều chỉnh tỷ lệ sang chiều ngang
const Card = memo(({
  item,
  index,
  activeIndex,
...
const Carousel3DLandscape = ({
  ...
  // Xử lý auto play - tự động xoay từ phải sang trái (ngược chiều kim đồng hồ)
  useEffect(() => {
    if (autoPlayEnabled && isCarouselActive) {
      autoPlayRef.current = setInterval(() => {
        ...
        // Điều chỉnh: di chuyển từ phải sang trái (giảm góc quay)
        const nextCard = (currentCard - 1 + totalCards) % totalCards;
  ...
  const panGestureHandler = useAnimatedGestureHandler({
    ...
    onActive: (event, context: any) => {
      if (isCarouselActive) {
        // Điều chỉnh: Khớp hướng vuốt và hướng xoay (vuốt sang phải, carousel quay phải)
        carouselRotation.value = context.startX + event.translationX * 0.2;
      }
    },
    onEnd: (event) => {
      if (isCarouselActive && Math.abs(event.velocityX) > 5) {
        // Điều chỉnh: Khớp hướng vuốt và hướng xoay
  ...
  // Di chuyển đến card trước - từ phải sang trái
  const goToPrev = useCallback(() => {
    const cardAngle = 360 / totalCards;
    const currentCard = Math.round(carouselRotation.value / cardAngle);
    // Card trước (giảm góc quay) - di chuyển từ phải sang trái
  ...
  // Di chuyển đến card tiếp theo - từ trái sang phải
  const goToNext = useCallback(() => {
    const cardAngle = 360 / totalCards;
    const currentCard = Math.round(carouselRotation.value / cardAngle);
    // Card tiếp theo (tăng góc quay) - di chuyển từ trái sang phải
...
const styles = StyleSheet.create({
  ...
  card: {
    position: 'absolute',
    width: 320, // Card theo chiều ngang: rộng hơn
    height: 200, // Card theo chiều ngang: thấp hơn
    alignItems: 'center',
    justifyContent: 'center',
...
export default Carousel3DLandscape; 