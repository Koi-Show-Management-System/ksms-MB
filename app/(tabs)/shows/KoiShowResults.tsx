...
import { getCompetitionCategories, CompetitionCategory } from '@/services/registrationService';
import { getCategoryResults, AwardResult, ResultMedia } from '@/services/resultService';
...
const CategorySelectItem = React.memo(({ item, onPress, isSelected }: { item: CompetitionCategory, onPress: (id: string, name: string) => void, isSelected: boolean }) => (
  <TouchableOpacity
...
const AwardWinnerCard = ({ item }: { item: AwardResult }) => {
  const getAwardStyle = (awardType: string | null | undefined) => {
    switch (awardType?.toLowerCase()) {
      case 'first': return styles.firstPlace;
      case 'second': return styles.secondPlace;
      case 'third': return styles.thirdPlace;
      case 'honorable': return styles.honorableMention; // Giả sử có giải khuyến khích
      default: return {};
    }
  };
  ...
  const getAwardIcon = (awardType: string | null | undefined) => {
    switch (awardType?.toLowerCase()) {
      case 'first': return { name: "emoji-events", color: "#FFD700" }; // Gold
      case 'second': return { name: "emoji-events", color: "#C0C0C0" }; // Silver
      case 'third': return { name: "emoji-events", color: "#CD7F32" }; // Bronze
      case 'honorable': return { name: "military-tech", color: "#888" }; // Honorable mention
      default: return { name: "help-outline", color: "#ccc" };
    }
  };
  ...
  const imageMedia = item.media?.find((m: ResultMedia) => m.mediaType === 'Image');

  return (
    <View style={[styles.winnerCard, awardStyle]}>
      <View style={styles.awardIconContainer}>
        <MaterialIcons name={awardIcon.name as any} size={28} color={awardIcon.color} />
        <Text style={styles.awardTypeText}>{item.awardName ?? item.awardType?.toUpperCase() ?? 'N/A'}</Text>
         {item.rank && <Text style={styles.rankText}>Hạng: {item.rank}</Text>}
      </View>
      <View style={styles.winnerInfo}>
        {imageMedia ? (
          <Image source={{ uri: imageMedia.mediaUrl }} style={styles.winnerImage} />
        ) : (
        ...
        <View style={styles.winnerTextContainer}>
          <Text style={styles.winnerName} numberOfLines={1}>{item.koiName ?? 'N/A'}</Text>
          <Text style={styles.winnerVariety} numberOfLines={1}>{item.variety ?? 'N/A'}</Text>
           <Text style={styles.winnerRegCode}>#{item.registrationNumber ?? item.registrationId.substring(0,6) ?? 'N/A'}</Text>
           <Text style={styles.winnerOwner}>Người đăng ký: {item.registerName ?? 'N/A'}</Text>
           <Text style={styles.winnerSize}>Kích thước: {item.koiSize?.toFixed(2) ?? 'N/A'} cm</Text>
           {item.finalScore !== undefined && item.finalScore !== null && (
             <Text style={styles.winnerScore}>Điểm: {item.finalScore.toFixed(2)}</Text>
           )}
        </View>
      </View>
    </View>
...
const KoiShowResults: React.FC<KoiShowResultsProps> = ({ showId }) => {
      ...
      try {
        ...
        if (response?.data?.items) {
          ...
          if (response.data.items.length > 0) {
             handleCategorySelect(response.data.items[0].id, response.data.items[0].name);
          } else {
  ...
  const fetchResults = useCallback(async (categoryId: string) => {
    ...
    try {
      ...
      if (response?.data) {
         ...
         const sortedResults = response.data.sort((a: AwardResult, b: AwardResult) => (a.rank ?? 999) - (b.rank ?? 999));
        ...
        setError(response?.message ?? "Không thể tải kết quả cho hạng mục này.");
      }
    } catch (err: any) {
      console.error("Lỗi khi tải kết quả:", err);
       setError(err?.response?.data?.message || err.message || "Đã xảy ra lỗi khi tải kết quả.");
    } finally {
  ...
  const renderResultItem = useCallback(({ item }: { item: AwardResult }) => (
    <AwardWinnerCard item={item} />
  ), []);
