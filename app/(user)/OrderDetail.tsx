...
const OrderDetail = () => {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const response = await api.get<OrderDetailResponse>(
        `/api/v1/ticket-order/get-order-details/${orderId}`
      );
  ...
  const fetchTickets = async (details = orderDetails) => {
    try {
      ...
      console.log(
        "Đang tải vé cho",
        details.length,
        "mục đơn hàng. Chi tiết đơn hàng IDs:",
        details.map((d) => d.id)
      );
      ...
      const ticketsPromises = details.map((detail) => {
        ...
        return api
          ...
          .catch((error) => {
            console.error(
              `Lỗi khi lấy vé cho orderDetailId ${detail.id}:`,
              error
            );
            return {
              data: {
      ...
      responses.forEach((response, index) => {
        ...
        console.log(
          `Response từ API cho orderDetailId ${details[index].id}:`,
          statusCode,
          response.data.message,
          response.data.data
            ? `Số vé: ${response.data.data.length}`
            : "Không có dữ liệu"
        );
      });
      ...
      if (allTickets.length === 0) {
        setTimeout(() => {
          alert(
            "Không tìm thấy vé nào cho đơn hàng này. Vui lòng liên hệ với ban tổ chức để được hỗ trợ."
          );
        }, 500);
      }
    } catch (err) {
  ...
  const navigateToTicketDetail = async (ticket: Ticket) => {
    try {
      ...
      if (!userFullName) {
        ...
        console.log(
          "Không tìm thấy tên người dùng trong AsyncStorage, sử dụng giá trị mặc định"
        );
      } else {
      ...
      router.push({
        ...
        params: {
          ...
          // Không truyền ticketNumber để không hiển thị mã vé
  ...
  const renderContent = () => {
      ...
      return (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptyText}>
            Không có thông tin chi tiết cho đơn hàng này
          </Text>
        </View>
      );
    }
    ...
    const showName =
      orderDetails[0]?.ticketType.koiShow.name || "Không xác định";
      ...
      <View style={styles.container}>
        <ScrollView style={styles.scrollView}>
          ...
          <View style={styles.orderHeaderContainer}>
            <Animated.View
              style={[styles.orderIdContainer, animatedOrderIdStyle]}>
              <Text style={styles.orderIdLabel}>Mã đơn hàng</Text>
              <TouchableOpacity
                onPress={toggleOrderIdDisplay}
                activeOpacity={0.6}>
                <Animated.Text
                  style={[styles.orderIdText, animatedOrderIdTextStyle]}>
                  {showFullOrderId
                    ? orderInfo.transactionCode.toUpperCase()
                    : formatId(orderInfo.transactionCode)}
                </Animated.Text>
          ...
          <View style={styles.sectionContainer}>
            ...
            {orderInfo?.createdAt && (
              <Text style={styles.orderDateText}>
                ...
                {new Date(orderInfo.createdAt).toLocaleTimeString("vi-VN", {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </Text>
        ...
        {orderInfo?.status &&
          orderInfo.status.toLowerCase() === "paid" &&
          tickets.length > 0 && (
                    ...
                    <View style={styles.ticketCardContent}>
                      <View style={styles.ticketCardInfo}>
                        <Text style={styles.ticketCardTitle}>
                          Mã vé: {item.id.toUpperCase()}
                        </Text>
                        {/* Tìm loại vé tương ứng với ticket này */}
                        {(() => {
                          const orderDetail = orderDetails.find(
                            (detail) => detail.id === item.orderDetailId
                          );
                          const ticketTypeName =
                            orderDetail?.ticketType?.name || "Không xác định";
                          return (
                        ...
                        <Text style={styles.ticketCardHint}>
